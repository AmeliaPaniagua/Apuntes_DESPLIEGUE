...
- Actualmente es estándar de facto en contenedores. Otros sistemas son LXD, LXC, RKT, ... (pregunta de examen)
...
```bash
docker login      --> logear
docker logout     --> cuando queremos salir
docker pull       --> bajar
docker push       --> incluir (subir)
docker run ...    --> ejecuta el contenedor
docker exec ...   --> ejecuta parte del contenedor  docker exec -it
docker images     --> para ver las imagenes que están descargadas en el disco duro
docker ps         --> para ver los contenedores que hay en la imagen
docker rmi ...    --> borrar imagenes
docker rm ...     --> borrar contenedores
docker stop       --> parar el contenedor
```
